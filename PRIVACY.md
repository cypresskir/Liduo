...
or video files. Liduo excludes its effect window from capture to prevent visual
feedback; its visible settings windows remain in the captured desktop.
Audio and microphone capture are disabled, and the cursor is excluded
...
writes a local status file containing the app version, lid angle, state flags,
frame counters, bounded presentation-timing summaries, and error messages.
It does not contain screen frames or audio. This file is
