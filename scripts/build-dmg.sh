#!/bin/zsh
set -euo pipefail
[[ $# -eq 1 ]] || { echo 'Usage: ./scripts/build-dmg.sh dist/Liduo-vVERSION-macos-arm64-adhoc.zip' >&2; exit 2; }
liduo_root="$(cd "$(dirname "$0")/.." && pwd)"
liduo_archive="${1:A}"
[[ -f "$liduo_archive" && "$liduo_archive" == *-macos-arm64-adhoc.zip ]] || {
  echo 'Expected an existing ad-hoc ZIP from build.sh adhoc.' >&2; exit 2
}
liduo_output="${liduo_archive%-adhoc.zip}.dmg"
...
[[ "${liduo_archive:t}" == "Liduo-v${liduo_version}-macos-arm64-adhoc.zip" ]] || { echo 'Archive name does not match app version.' >&2; exit 2; }
codesign --verify --deep --strict "$liduo_app"
codesign -d --verbose=2 "$liduo_app" 2> "$liduo_stage/signature.txt"
/usr/bin/grep -q '^Signature=adhoc$' "$liduo_stage/signature.txt" || { echo 'Expected an ad-hoc app. The bundled guide describes an unnotarized build.' >&2; exit 2; }
...
echo 'Ad-hoc app: first launch may require Open Anyway in macOS Privacy & Security.'
